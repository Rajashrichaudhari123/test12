...
(function(){
	...
	csvjson.csv2json = function(csvdata, args) {
		...
		if (!header) {
			for (var i = 0; i < csvheaders.length; i++) {
		  	csvheaders[i] = i.replace(removeNewLineRegExp, "");
			};
		}

		for (var i = 0; i < csvheaders.length; i++) {
			csvheaders[i] = csvheaders[i].replace(removeNewLineRegExp, "");
		};
	...
	*/
	//siva, added flag
	csvjson.json2csv = function(jsondata, args, flag) {
		args = args || {};
		var delim = isdef(args.delim) ? args.delim : ",";
		var textdelim = isdef(args.textdelim) ? args.textdelim : "";
		...
		// Add the headers
		for(var h=0; h<jsondata.headers.length; h++) {
			...
			if (jsondata.rows.hasOwnProperty(r)) {
				...
				for(var h=0; h<jsondata.headers.length; h++) {
					if (jsondata.headers.hasOwnProperty(h)) {
						...
						} else if(typeof(data) == "undefined" && flag){
							ret += textdelim + '-' + textdelim + delim;
						}else {
